...
(define (set-variable-value! var val env)
  (define (env-loop env)
    (define (scan frame)
      (if (set-frame-binding! var val frame) true
          (env-loop (enclosing-environment env))))
    (if (eq? env the-empty-environment)
...
(define (set-frame-binding! var new-val frame) ;set new-val of variable in the frame
  (define (loop frame)
    (if (null? frame)
        false
        (let ((pair (car frame)))
...
;;; exer 4.11
;; functions need to be changed are :
...
; define-variable! and set-variable-value! are also need to be changed since they are coupled with the implementation of frame
