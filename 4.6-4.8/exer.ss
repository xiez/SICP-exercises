...
;;   (* x z)
;;   (+ x z))
;; can be transformed to nested `let`,
;; (let ((x 3))
;;   (let ((y (+ x 2)))
;;     (let (( z (+ x y 5)))
;;       (* x z)
...
;;; it's sufficient to add a clause to eval whose action is
;; (eval (let*->nested-lets exp) env)
(eval
 '(let* ((x 3)
         (y (+ x 2))
...
;;        fib-iter                         ;<-- name
;;      ((a 1)                             ;<-- varlist
;;       (b 0)                             ;(a b count) <-- variables
;;       (count n))                        ;(1 0 n) <-- initial-vals
;;      (if (= count 0)                    ;<-- body
;;          b
;;          (fib-iter (+ a b) a (- count 1))))
;;   )
